...
while True:
    ...
    oven = requests.get('https://circusofthings.com/ReadValue', {'Key':oven_key, 'Token':token})
    #Lagrer info i python-vennlig format og ekstrakter "Value" som vi er mest interessert i 
    oven = oven.json()['Value']
    
    #Sjekker om noen har slått på TVen
    TV = requests.get('https://circusofthings.com/ReadValue', {'Key':TV_key, 'Token':token})
    #Lagrer info i python-vennlig format og ekstrakter "Value" som vi er mest interessert i 
    TV = TV.json()['Value']
    
    #Sjekker om oppvaskmaskinen er på 
    dishwasher = requests.get('https://circusofthings.com/ReadValue', {'Key':dishwasher_key, 'Token':token})
    #Lagrer info i python-vennlig format og ekstrakter "Value" som vi er mest interessert i 
    dishwasher = dishwasher.json()['Value']
    ...
    if oven > 0:
        consumption_so_far += 0.85
    if TV > 0:
        consumption_so_far += 0.0085
    if dishwasher > 0:
        consumption_so_far += 0.167
    ...
    if now == today_midnight:  #Dersom det er midnatt nå
        ...
        consumption_so_far = 40             #restarter strømmåleren 
